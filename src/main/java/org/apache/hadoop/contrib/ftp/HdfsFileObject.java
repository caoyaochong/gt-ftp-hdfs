...
//			dfs.setOwner(path, user.getName(), user.getMainGroup());
			dfs.setPermission(path, new FsPermission("747")); //修改创建文件的权限
			return true;
		} catch (IOException e) {
			e.printStackTrace();
			return false;
	...
	public FileObject[] listFiles() {
		...
		try {
			...
			for (int i = 0; i < fileStats.length; i++) {
				...
				// System.err.println("getOwnerName " + this.getOwnerName());
				// System.err.println("path " + path);
//				if (user.getName().equals(this.getOwnerName())) {
//					fileObjects[i] = new HdfsFileObject(fileStats[i].getPath().toString(), user);
//				}
					
				if (path.toString().startsWith("/user/" + user.getName())) {
					fileObjects[i] = new HdfsFileObject(fileStats[i].getPath().toString(), user);
				}
			}
			return fileObjects;
